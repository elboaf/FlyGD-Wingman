...
**Status:** Engineering implementation, review and verification are complete, including the capture-entry and screenshot-isolation corrections in `4333963c`. The branch integrates main at `f9795e15` after #225. Fresh post-fix Linux verification passed 12,347 tests with 13 Windows-only skips; polish is clear and actual CodeRabbit review is complete. These checked engineering steps do not claim Windows/WebView2/native operator acceptance, which remains outstanding. See the [implementation notes](preview-character-markers-implementation-notes.md) for exact evidence and historical checkpoints.
...
**Spec:** [Approved shared design](preview-identification-design.md), especially “#215 — per-character recognition”; #214's settled containment remains authoritative. Design measurements and implementation evidence: [implementation notes](preview-character-markers-implementation-notes.md).
...
- Work only in `.worktrees/preview-character-markers`, branch `feature/preview-character-markers`, original implementation base `2404917a1eb7771649e9473103f3561bd618cd37`, now integrated onto `f9795e15379a900fec5ca48de67a1d17522b886c`. Do not alter #214's settled scope.
- Implementation and scoped review corrections are complete. Further worker edits or commits require explicit dispatch authorization. Coordinator owns final verification and publication gates, including `/polish --fix` and actual CodeRabbit review. No merge or issue action is authorized.
...
**Files and ownership (implemented Task 1 scope):**
...
- [x] **3. Run RED, then add only the model/settings/store implementation.** Run `python -m pytest tests/test_preview_labelmarkers.py tests/test_settings_preview.py tests/test_preview_store.py -q` through the dedicated `uv run --no-sync`; confirm failures name the missing marker behavior. Implement the interfaces above, derive choices with `[{'key': '', 'label': 'None'}] + [{'key': key, 'label': value[0]} for key, value in MARKER_PALETTE.items()]`, reconstruct the map in `validated_preview`, and union its keys into `_protected`. Rerun those tests GREEN plus `tests/test_settings.py` for the exact-default fixture.
