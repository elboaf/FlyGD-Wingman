...
This release is deliberately local-only. The existing authGD protocol remains version 1 and continues publishing outgoing DPS plus `SCRAM/POINT` only. Shared incoming DPS follows after the unmerged shared-fleet receive/display work lands and authGD can evolve its strict schema compatibly.
...
The exact values use the existing tabular monospace treatment. The unit is established by the `DAMAGE` heading and Wingman's existing Fleet Bar context, so the narrow cell shows bare whole numbers rather than repeating `dps` twice per row.
...
Outgoing uses the quiet neutral text/rail vocabulary. Incoming uses the same warm `--warn` family as active EWAR, grouping received damage with received effects without using the error or destructive-action colors. Labels, position, numbers, and direction carry meaning independently of color.
...
Rail updates use a 150 ms transform transition with an ease-out-quart curve, so one-second snapshot changes remain trackable without animating layout. Under `prefers-reduced-motion: reduce`, fills update without a transition.
...
`incoming_damage` already carries a parsed amount through `ParsedFact` and `CombatFact`. Implementation must add metric-grade fixtures and parsing tests for verified comma-separated and malformed amount shapes rather than assuming alert-level event recognition proves numeric fidelity.
...
Incoming metric errors use direction-specific diagnostic text so a rejected timestamp identifies whether incoming or outgoing data failed. A later accepted metric fact may clear the shared metric diagnostic under the existing Fleet Metrics policy.
...
- exact window boundaries, decay, future clamp, and future rejection;
...
- `prefers-reduced-motion` where the browser harness can emulate it.
...
5. Each rail independently normalizes against the highest numeric value among the page's visible rows; exact numbers remain visible.
