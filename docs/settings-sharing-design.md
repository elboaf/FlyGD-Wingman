...
Status: phase 1 implemented, with local verification and polish corrections recorded in the [verification record](probe-formation-sharing-verification.md). Independent task and whole-branch reviews are complete with no outstanding code findings; Windows/WebView2, real-clipboard, and live-EVE acceptance remain open. The phase-1 requirements below are unchanged; see the [phase-1 implementation plan](probe-formation-sharing-plan.md).
