...
`filename` is the only stretching column, so it is the only one that grows on a
wider window — see "Narrow windows" for what that costs at the floor.
...
410px, but only the minimums of *stretching* columns are ever reachable — and
`filename` is the only one. Two facts, both measured against a real window
rather than reasoned about, decide the rest:

1. `ttk.Treeview` distributes a width deficit across its **stretching** columns
   only; a `stretch=False` column keeps its configured width. So the list's
   real floor is the sum of the *preferred* widths of the five fixed columns
   (360px) plus `filename`'s 120px minimum, plus a 10px inset inside the tree:
   **490px of viewport**. The inset is a constant 10px at 96, 144 and 192 dpi —
   it does not scale.
2. The tree viewport at a 750px window is **381px**, not the 380–420px an
   earlier draft estimated as adequate, so **the window minimum is raised from
   750 to 860**, which gives a 491px viewport. Verified by `bbox` at 100%, 150%
   and 200%: zero clipped columns at the floor and at every wider step.

Making `date`, `size` and `duration` stretch so their minimums became reachable
was tried and rejected. `stretch` is symmetric in ttk, so those columns then
took an equal share of a *surplus* too — a 1920px window handed `Size` 344px to
right-align "1.0 KB" in — which repealed "a wider window widens the filename
column" at every width, and undoing it required a `<Configure>` handler
switching regimes at a threshold width. 60px more minimum window width buys the
same result with no machinery.
...
its floor shows a short filename column — every column still present, none
clipped — and that is accepted.
...
- A 300px panel at 100% is comfortable at the default 1350px width. It is
  ...
  minimums did fit at 750px; measurement says the viewport there is 381px, and
  that only `filename`'s minimum is reachable at all, so the floor was raised to
  860px — 491px of viewport against the 490px the columns need. The result at
  the floor is a short filename column, but complete.
