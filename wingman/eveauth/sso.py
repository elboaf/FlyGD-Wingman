...
Shared by every EVE-authenticated capability. `authorize_url` takes the
scope set to request as an explicit, required argument -- there is no
"every scope" default anywhere in this module, so a capability that wants
a narrower or wider consent screen changes what IT passes, never what
...
def authorize_url(pkce: Pkce, scopes: Iterable[str]) -> str:
    """Build the URL the browser is sent to, for exactly *scopes*.

    There is no default scope set. A caller names the capability's own
    scopes explicitly (`application.SKILLS_SCOPES`, `application.
    FITTINGS_SCOPES`, or any other explicit set) every time -- widening
    what one capability requests must be a deliberate change to that
    capability's own constant, never a side effect of calling this
    function with nothing.
    """
    # frozenset first, so a caller-supplied duplicate or a generator
    # exhausted by an earlier pass over the same iterable cannot produce a
    # shorter or differently-ordered scope list than what was intended.
