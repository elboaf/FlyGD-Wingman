...
PICKER_CLASS = "WingmanPreviewRegionPicker"
PICKER_MAX = (1200, 800)
...
def _ensure_class(libs):
    ...
    _register_class(libs)
    _CLASS_REGISTERED = True


def _register_class(libs, class_name=PICKER_CLASS):
...
class RegionPicker:
    ...
    class_name = PICKER_CLASS

    @staticmethod
    def _ensure_class(libs):
        _ensure_class(libs)

    def __init__(self, libs, client, on_confirm, on_cancel, caption, strict_size=True):
        self._libs = libs
        self.client = client
        self.caption = caption
        self._strict_size = strict_size
    ...
    def create(
        ...
        *,
        ...
        """
        self = cls(libs, client, on_confirm, on_cancel, caption, strict_size)
        try:
            self._source_size = _client_size(libs, client.hwnd)
            cls._ensure_class(libs)
            self.hwnd = _require(
    ...
    def _layout(self, message=None):
        ...
        if source != self._source_size:
            if self._strict_size:
                self.cancel("Source size changed. Select the region again.")
                return
    ...
    def _fail(self, exc):
        logger.warning(
            "Region picker %r src=0x%x: %s", self.caption, self.client.hwnd, exc
        )
        self.cancel(str(exc))

    def _resources_pending(self):
        return bool(
            self.hwnd is not None
            or self._overlay_hwnd is not None
            or self._controls
            or self._thumb is not None
            or self._fonts
        )

    def _destroy_owned(self, hwnd):
        return bool(
            self._libs.user32.DestroyWindow(hwnd)
            or not self._libs.user32.IsWindow(hwnd)
        )

    def _close_resources(self, *, destroy_picker=True):
        if self._closing_resources:
            return
        ...
        try:
            ...
            if self._overlay_hwnd is not None and self._destroy_owned(
                self._overlay_hwnd
            ):
                self._overlay_hwnd = None
            for ident, hwnd in tuple(self._controls.items()):
                if self._destroy_owned(hwnd):
                    self._controls.pop(ident, None)
            if not self._controls:
                self._font = None
            self._release_fonts()
            if (
                self._thumb is not None
                and self._libs.dwmapi.DwmUnregisterThumbnail(self._thumb._handle) == 0
            ):
                self._thumb._handle = None
                self._thumb = None
            # Parent destruction invalidates children; retain the intact owner
            # until their individual releases and DWM release have succeeded.
            if (
                self.hwnd is not None
                and destroy_picker
                and not self._controls
                and self._overlay_hwnd is None
    ...
    def _notify_pending_cancel(self):
        if self._completed and self._resources_pending():
            self._close_resources()
        self._release_fonts()
        if self._resources_pending() and self._completed:
            return
        if self._pending_confirm is not None:
