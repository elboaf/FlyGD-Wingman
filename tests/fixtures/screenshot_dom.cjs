// The screenshot harness's deliberately small DOM subset. The factory is
// closure-free so workers can evaluate the same source inside each request VM;
// direct Node tests use the CommonJS createDOM instance from the host realm.
  ...
  function createDOM(page) {
    ...
    class Element {
      ...
      insertBefore(el, before) {
        ...
        this.children.splice(index, 0, el); el.parentNode = this; return el;
      }
      ...
      set innerHTML(text) {
        if (text !== '') throw new Error('screenshot DOM only supports clearing innerHTML');
        this.textContent = '';
      }
...
const DOM_FACTORY_SOURCE = `(${createDOMFactory.toString()})()`;
const createDOM = createDOMFactory();

module.exports = {createDOM, DOM_FACTORY_SOURCE};
