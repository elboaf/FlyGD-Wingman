...
async function profilesWorkbenchMain() {
  ...
  if (scenario === 'profiles-workbench-scope') {
    ...
    assert.match(WM.el('es-copy-scope-note').textContent, /bundled settings codec.*missing/i);
    assert.match(WM.el('es-copy-scope-note').textContent, /replace.*whole.*file/i);
    assert.match(WM.el('es-copy-scope-note').textContent, /reinstall.*Wingman.*installer/i);
    assert.doesNotMatch(WM.el('es-copy-scope-note').textContent, /unchanged|excluded|keep/i);
    assert.ok(WM.el('es-copy-scope-note').className.split(/\s+/).includes('warn'));
    const scopeCommit = WM.el('es-copy-scope-commit');
    assert.equal(scopeCommit.textContent, 'Whole settings file');
    assert.equal(scopeCommit.hidden, false);
    assert.ok(scopeCommit.className.split(/\s+/).includes('warn'));
    assert.equal(WM.el('es-commit-context').contains(scopeCommit), true, 'scope stays beside the action outside the work scroller');
    ...
    ordinaryCopies.at(-1).resolve(false); await tick();
    await setKind('accounts');
    assert.equal(scopeCommit.textContent, 'Whole settings file', 'account mode has the same whole-file fallback');
    assert.equal(scopeCommit.hidden, false);
    click('es-all'); click('es-copy');
    assert.deepEqual(plain(ordinaryCopies.at(-1).args), ['account-A', ['account-B']], 'account fallback still omits the groups argument');
    ordinaryCopies.at(-1).resolve(false); await tick();
    await repaint(data);
    assert.equal(scopeCommit.textContent, '', 'restored selective capability retires the fallback scope');
    assert.equal(scopeCommit.hidden, true);
    assert.equal(scopeCommit.className.split(/\s+/).includes('warn'), false);
    assert.equal(WM.el('es-copy-scope-note').className.split(/\s+/).includes('warn'), false);
    assert.equal(WM.el('es-copy-scope').hidden, false);
    assert.match(WM.el('es-copy-scope-note').textContent, /Unchecked groups stay unchanged/);
    assert.doesNotMatch(WM.el('es-copy-scope-note').textContent, /missing|reinstall/i);
    click('es-all'); click('es-copy');
    assert.deepEqual(plain(ordinaryCopies.at(-1).args), ['account-A', ['account-B'], ['audio']], 'restored selective capability sends the selected account groups');
