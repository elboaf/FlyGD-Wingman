...
(async () => {
  ...
  // Reset leaves live Size actions with configured defaults, not blank dialogs.
  const configured = (await api.get_settings()).settings.preview;
  assert.equal(configured.width, 480); assert.equal(configured.height, 300);
  await api.set_preview_size(a, 700, 450);
  const beforeReset = await api.get_preview_hotkey_state();
  assert.ok(beforeReset.layout_sources.length > 0);
  const receipt = await api.reset_preview_layouts();
  assert.deepEqual(JSON.parse(JSON.stringify(receipt)), {applied: true, persisted: true, error: null});
  const reset = await api.get_preview_hotkey_state();
  assert.equal(reset.layout_sources.length, 0);
  assert.deepEqual([...reset.sizable], [...initial.characters]);
  assert.equal(Object.keys(reset.sizes).length, reset.sizable.length);
