...
from obs_youtube_uploader import library, tooltip
...
def test_unreadable_duration_explains_itself():
    ...
    help_text = tooltip.tooltip_for_cell("duration", "?")
...
def test_pending_probe_is_distinguished_from_a_failed_one():
    ...
    pending = tooltip.tooltip_for_cell("duration", "…")
    failed = tooltip.tooltip_for_cell("duration", "?")
    assert pending != failed
    assert "Measuring" in pending


def test_link_glyph_explains_both_of_its_gestures():
    help_text = tooltip.tooltip_for_cell("link", "↗")
    assert "Double-click" in help_text
    assert "right-click" in help_text.lower()


def test_an_ordinary_duration_gets_no_tooltip():
    assert tooltip.tooltip_for_cell("duration", "59:58") is None


def test_an_empty_link_cell_gets_no_tooltip():
    assert tooltip.tooltip_for_cell("link", "") is None


def test_unknown_columns_get_no_tooltip():
    assert tooltip.tooltip_for_cell("filename", "a.mkv") is None
    assert tooltip.tooltip_for_cell("nonsense", "?") is None
...
def test_the_keys_match_what_video_info_actually_renders(probed, duration,
    ...
    got = tooltip.tooltip_for_cell("duration", info.duration_str)
