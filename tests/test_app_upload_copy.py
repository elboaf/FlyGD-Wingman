...
def info(name="a.mkv", size=1_000_000, duration=60.0, probed=True):
    ...
    return library.VideoInfo(
        path=Path(name), mtime=0.0, size=size, duration=duration, probed=probed
    )
...
def test_destination_names_the_channel_once_it_is_known():
    assert (
        copy_mod.format_destination("Zoolanders", "unlisted")
...
def test_destination_is_honest_before_the_first_upload():
    ...
    assert (
        copy_mod.format_destination("", "public")
...
def test_title_hint_warns_that_a_batch_is_numbered():
    ...
    assert (
        copy_mod.format_title_hint(10, stitch=False)
