...
def test_rejects_non_utc_datetime(tmp_path):
    ...
    with pytest.raises(ValueError):
        combatlog.select_logs(
            tmp_path,
            datetime.datetime(2026, 8, 20, 16, 0, tzinfo=other),
            _utc(2026, 8, 20, 21, 0),
        )
...
def test_selects_a_log_overlapping_the_window(tmp_path):
    _log(
        tmp_path,
        "20260820_204250_1.txt",
        "Pilot A",
        "2026.08.20 20:42:50",
        _epoch(_utc(2026, 8, 20, 21, 55)),
    )
    sel = combatlog.select_logs(
        tmp_path, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30)
    )
...
def test_reversed_bounds_are_swapped(tmp_path):
    ...
    sel = combatlog.select_logs(
        tmp_path, _utc(2026, 8, 20, 21, 30), _utc(2026, 8, 20, 21, 0)
    )
    assert [s.listener for s in sel.logs] == ["Pilot A"]
...
def test_skips_log_that_ended_before_the_window(tmp_path):
    ...
    _log(
        tmp_path,
        "20260819_100000_1.txt",
        "Old Pilot",
        "2026.08.19 10:00:00",
        _epoch(_utc(2026, 8, 19, 11, 0)),
    )
    sel = combatlog.select_logs(
        tmp_path, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30)
    )
...
def test_skips_log_that_started_after_the_window(tmp_path):
    _log(
        tmp_path,
        "20260821_100000_1.txt",
        "Future Pilot",
        "2026.08.21 10:00:00",
        _epoch(_utc(2026, 8, 21, 11, 0)),
    )
    sel = combatlog.select_logs(
        tmp_path, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30)
    )
...
def test_padding_pulls_in_a_log_just_outside_the_window(tmp_path):
    ...
    _log(
        tmp_path,
        "20260820_205600_1.txt",
        "Edge Pilot",
        "2026.08.20 20:56:00",
        _epoch(_utc(2026, 8, 20, 20, 58)),
    )
    sel = combatlog.select_logs(
        tmp_path, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30)
    )
...
def test_caps_at_max_files_and_reports_dropped(tmp_path):
    for i in range(5):
        _log(
            tmp_path,
            f"20260820_2100{i:02d}_{i}.txt",
            f"Pilot {i}",
            "2026.08.20 21:00:00",
            _epoch(_utc(2026, 8, 20, 21, 10 + i)),
        )
    sel = combatlog.select_logs(
        tmp_path, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30), max_files=3
    )
...
def test_cap_keeps_the_newest_by_last_write(tmp_path):
    ...
    sel = combatlog.select_logs(
        tmp_path, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30), max_files=2
    )
...
def test_missing_directory_yields_nothing(tmp_path):
    sel = combatlog.select_logs(
        tmp_path / "nope", _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30)
    )
...
def test_stat_guard_still_excludes_a_log_that_would_be_excluded_anyway(tmp_path):
    ...
    _log(
        tmp_path,
        "20260601_100000_1.txt",
        "Ancient Pilot",
        "2026.06.01 10:00:00",
        _epoch(_utc(2026, 6, 1, 11, 0)),
    )
    sel = combatlog.select_logs(
        tmp_path, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30)
    )
    ...
    for i in range(count):
        ...
        logs.append(
            combatlog.SelectedLog(
                ...
                span_end=_utc(2026, 8, 20, 21, 10 + i),
            )
        )
...
def test_archive_contains_every_selected_log(tmp_path):
    ...
    combatlog.build_archive(
        sel, out, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30)
    )
...
def test_manifest_records_dropped_count(tmp_path):
    ...
    out = tmp_path / "out.zip"
    result = combatlog.build_archive(
        sel, out, _utc(2026, 8, 20, 21, 0), _utc(2026, 8, 20, 21, 30)
    )
...
def test_result_reports_characters_sorted_and_deduped(tmp_path):
    ...
    q = _log(
        tmp_path,
        "20260820_210001_2.txt",
        "Zed",
        "2026.08.20 21:00:00",
        _epoch(_utc(2026, 8, 20, 21, 11)),
    )
    r = _log(
        tmp_path,
        "20260820_210002_3.txt",
        "Alice",
        "2026.08.20 21:00:00",
        _epoch(_utc(2026, 8, 20, 21, 12)),
    )
    logs = [
        combatlog.SelectedLog(
            path=x,
    ...
    out = tmp_path / "out.zip"
    result = combatlog.build_archive(
        combatlog.Selection(logs=logs, dropped=0),
        out,
        _utc(2026, 8, 20, 21, 0),
...
def test_dropped_note_singular():
    assert (
        combatlog.dropped_note(1) == f"1 older log omitted (cap {combatlog.MAX_FILES})"
    )
...
def test_summarize_archive_omits_drop_clause_when_nothing_dropped():
    ...
    assert (
        summary
        == "Combat logs 2026-08-20 21:00\u201321:30 UTC \u00b7 2 file(s) \u00b7 Alice, Zed"
    )
