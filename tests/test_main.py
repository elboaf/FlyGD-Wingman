...
def test_build_fleet_sharing_worker_is_platform_neutral_and_starts_stopped(
    ...
    """An empty install may probe metadata, never create identity or consent."""
...
def test_main_retains_the_single_sharing_owner_for_disabled_startup():
    """Composition guard; executable lifecycle cases live in test_api_fleetsharing."""
    source = inspect.getsource(main_mod.main)
    assert "fleet_sharing=sharing_worker" in source
    assert "telemetry_factory=" in source
    assert "api._start_fleet_sharing()" in source
